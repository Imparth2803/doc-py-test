...
export function buildDownloadFilename(document: { name: string; originalName?: string; mimeType?: string }): string {
  const displayTitle = (document.name || 'Document').trim();
  ...
  if (!ext) {
    ext = getExtensionFromMime(document.mimeType);
  }
  
  // 3. Check if name already ends with .ext
