...
export function TreeView() {
  ...
  const handleAddDirect = (folder: string) => {
  ...
  const treeData = useMemo(() => {
    ...
    // Initialize ALL folders so they are always visible
    const allKnownFolders = [...ALL_FOLDERS, ...customFolders];
    allKnownFolders.forEach(f => {
    ...
    documents.forEach(doc => {
      ...
      const folderMap = rootMap.get(folder)!;
      
      entities.forEach(entity => {
         if (!folderMap.has(entity)) folderMap.set(entity, []);
    ...
    <div className="min-h-screen bg-[#fafafc] flex flex-col font-sans relative pb-24">
      {isAnalyzing && (
        <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-white/80 backdrop-blur-sm">
          ...
          <p className="text-gray-500 text-lg">Extracting details and adding to {targetFolder}...</p>
        </div>
      )}
      <TopNav activeTab="tree" />
      
      <main className="flex-1 w-full max-w-5xl mx-auto p-6 md:p-10 space-y-8">
         <div className="mb-4">
           <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Navigation Tree</h1>
         ...
         <div className="bg-white rounded-[24px] border border-gray-100 shadow-sm p-4 sm:p-6 overflow-hidden">
            ...
            {categories.map((catKey) => {
               ...
               let totalDocsInCat = 0;
               catFolders.forEach(f => {
                  const entMap = treeData.get(f);
                  if (entMap) {
                    totalDocsInCat += Array.from(entMap.values()).reduce((sum: number, docs: any) => sum + (docs as any[]).length, 0) as number;
                  }
               });
               ...
               return (
                  <div key={catKey} className="mb-2">
                     ...
                     >
                       <ChevronRight className={cn("text-gray-400 transition-transform", isCatExpanded && "rotate-90")} size={20} />
                       <Folder className={cn(isCatExpanded ? "text-blue-500 fill-blue-100" : "text-gray-400")} size={22} />
                       <span className="font-semibold text-gray-800 text-lg">{catKey}</span>
                       <span className="ml-auto text-sm font-bold text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">{totalDocsInCat}</span>
                     </div>
                        ...
                        {isCatExpanded && (
                           ...
                           >
                              {catFolders.map(folder => {
                                 const entitiesMap = treeData.get(folder) || new Map();
                                 const folderKey = folder;
                                 const isFolderExpanded = expandedFolders.has(folderKey);
                                 const totalDocsInFolder = Array.from(entitiesMap.values()).reduce((sum: number, docs: any) => sum + (docs as any[]).length, 0) as number;

                                 return (
                                   <div key={folder} className="mb-1">
                                      <div 
                                        className={cn("flex items-center gap-3 p-2 rounded-lg transition-colors group", totalDocsInFolder > 0 ? "cursor-pointer hover:bg-gray-50" : "cursor-default")}
                                        onClick={() => totalDocsInFolder > 0 && toggleFolder(folderKey)}
                                      >
                                        <ChevronRight className={cn("text-gray-400 transition-transform", isFolderExpanded && "rotate-90", totalDocsInFolder === 0 && "opacity-0")} size={16} />
                                        <div className="w-6 h-6 rounded bg-purple-50 flex items-center justify-center shrink-0">
                                            <Folder className="text-purple-600" size={12} strokeWidth={3} />
                                        </div>
                                        <span className={cn("font-semibold text-[15px]", totalDocsInFolder > 0 ? "text-gray-700" : "text-gray-400")}>{folder}</span>
                                        <div className="ml-auto flex items-center gap-2">
                                           <button
                                              onClick={(e) => { e.stopPropagation(); handleAddDirect(folder); }}
                                           ...
                                           <span className="text-xs font-bold text-gray-400 group-hover:text-gray-500 w-12 text-right">{totalDocsInFolder} docs</span>
                                        </div>
                                      </div>

                                      <AnimatePresence>
                                          {isFolderExpanded && totalDocsInFolder > 0 && (
                                            <motion.div
                                                initial={{ height: 0, opacity: 0 }}
                                                animate={{ height: 'auto', opacity: 1 }}
                                                exit={{ height: 0, opacity: 0 }}
                                            ...
                                            >
                                               {Array.from(entitiesMap.entries()).sort(([a], [b]) => a.localeCompare(b)).map(([ent, docs]) => {
                                                  const entityKey = `${folder}-${ent}`;
                                                  const isEntExpanded = expandedEntities.has(entityKey);
                                                  ...
                                                  return (
                                                    <div key={ent} className="mb-1">
                                                       ...
                                                       >
                                                          <ChevronRight className={cn("text-gray-400 transition-transform", isEntExpanded && "rotate-90")} size={14} />
                                                          <User className="text-blue-500" size={14} />
                                                          <span className="font-semibold text-gray-700 text-sm">{ent}</span>
                                                       </div>
            ...
            {customFolders.length > 0 && (
               <div className="mb-2 mt-6">
                  <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest px-4 mb-2">Custom Folders</h3>
